...
本地认证安全摘要已记录登录成功/失败、退出、未认证访问、角色拒绝和 CSRF 拒绝，并关联 BFF 请求 ID。摘要不保存密码、用户名输入、Cookie、会话令牌、CSRF 值、查询参数、请求正文、来源 IP 或客户端指纹；没有已落库身份的外部/测试身份统一显示为“未识别身份”。每条 SQLite 审计事件含上一条事件摘要的 SHA-256 哈希，读取时重算并在不一致时拒绝继续追加。该链能发现内容或中间顺序改变，但无法单独防止尾部删除或拥有数据库写权限的攻击；生产仍需补齐可信来源、完整权限变更覆盖、受限数据库权限、外部/密钥保护的完整性锚点与跨服务审计。
