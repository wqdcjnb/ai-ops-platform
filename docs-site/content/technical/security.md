...
当前本机 BFF 的最小实现将会话令牌和 CSRF 令牌仅以哈希形式写入 SQLite；浏览器会话 Cookie 为 `HttpOnly` 与 `SameSite=Strict`，所有已登录写请求还必须携带与 CSRF Cookie 匹配的请求头。该机制用于本机原型防护，不能替代生产环境的 MFA、企业身份源、会话清理、设备管理或完整操作审计。
