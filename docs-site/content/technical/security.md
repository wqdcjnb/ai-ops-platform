...
5. 给令牌配置人员、部门、用途、单一绑定模型、额度、有效期和状态。
...
- 每人一个账号和一个或多个用途令牌。
- 每个 Key 默认只允许其绑定模型；需要多个模型时签发多个 Key，网关在上游请求前校验绑定关系。
...
当前本机 BFF 的最小实现将会话令牌和 CSRF 令牌仅以哈希形式写入 SQLite；浏览器会话 Cookie 为 `HttpOnly` 与 `SameSite=Strict`，所有已登录写请求还必须携带与 CSRF Cookie 匹配的请求头。BFF 启动及每次成功登录时会删除已过期会话，并在撤销满 24 小时后删除会话哈希；每次执行仅记录触发来源、完成时间和删除数量，不记录令牌、用户或来源 IP。该机制用于本机原型防护，不能替代生产环境的 MFA、企业身份源、设备管理或完整操作审计。

本地认证安全摘要已记录登录成功/失败、退出、未认证访问、角色拒绝和 CSRF 拒绝，并关联 BFF 请求 ID。摘要不保存密码、用户名输入、Cookie、会话令牌、CSRF 值、查询参数、请求正文、来源 IP 或客户端指纹；没有已落库身份的外部/测试身份统一显示为“未识别身份”。每条 SQLite 审计事件含上一条事件摘要的 SHA-256 哈希，读取时重算并在不一致时拒绝继续追加；单行检查点同时保存事件数和链头，用于发现尾部删除。拥有数据库写权限的攻击者仍可同时重写事件和检查点；生产仍需补齐可信来源、完整权限变更覆盖、受限数据库权限、外部/密钥保护的完整性锚点与跨服务审计。

部门负责人读取统一入口、总览、人员、Key、额度、用量和告警时，BFF 依据已认证会话的 SQLite 部门归属在服务端过滤列表、汇总、树节点和详情；跨部门人员、Key、调用记录与告警详情按不存在处理，不能靠前端筛选或直接 URL 绕过。该规则仅覆盖当前本地模拟数据和已实现的接口，真实组织同步、跨部门授权审批和数据范围变更审计仍未接入。
...
业务确需对话审计时，使用独立采集层和独立审计存储，不把正文混入 New API、CPA 或反向代理的普通日志。采集默认关闭，只能按人员、用途或 Key 开启，并同时设置到期时间。正文入库前完成字段级过滤，入库后使用应用层加密；加密密钥保存在数据库之外。

普通管理员只能读取调用元数据。超级管理员查看脱敏正文时必须填写原因；查询、查看、复制、导出和删除操作全部再次记录。留存到期后删除正文并保留不含内容的删除证明。

如确需短期保存更详细内容用于排障，必须限定到单个请求 ID、自动到期并完成脱敏。任何模式都不得保存认证 Header、完整 Key、OAuth Token或未经批准的原始文件。
...
- 令牌撤销后超过 60 秒仍可继续调用。
- 普通用户可以指定任意上游地址或绕过 Key 的模型绑定。
- 生产环境依赖个人 Pro OAuth 或浏览器会话。
- 网关镜像使用浮动版本且没有回滚版本。
- 数据库和备份未加密或直接暴露到公网。
- 未启用审计的人员或 Key 仍被保存了请求、回答或工具正文。
- 普通管理员能够查看对话正文，或正文查看/导出没有生成访问审计。
...
13. 分别使用开启和关闭审计的测试 Key，验证只有开启者产生加密内容记录。
14. 在对话中加入测试密钥、手机号、地址和订单号，验证正文展示与存储均按策略脱敏。
15. 验证普通管理员不能读取正文；超级管理员查看、搜索、导出和删除均产生独立访问审计。
16. 验证留存到期后正文被清除，且不影响 New API 用量记录和费用统计。

## 11. 推荐实施顺序

1. 使用官方 API 或获得授权的 Provider 完成最小链路。
2. 部署独立 HTTPS 调用域名和内网管理域名。
3. 完成人员独立令牌、Key 单模型绑定、硬额度和撤销机制。
4. 关闭请求正文日志，完成凭据统一脱敏。
