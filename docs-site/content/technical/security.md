...
本地认证安全摘要已记录登录成功/失败、退出、未认证访问、角色拒绝和 CSRF 拒绝，并关联 BFF 请求 ID。摘要不保存密码、用户名输入、Cookie、会话令牌、CSRF 值、查询参数、请求正文、来源 IP 或客户端指纹；没有已落库身份的外部/测试身份统一显示为“未识别身份”。这只提供本机审计线索，生产仍需补齐可信来源、完整权限变更覆盖、追加完整性与哈希链验证。
