...
平台解决三个问题：给每名运营人员发放独立 API Key；按人员、部门和业务用途分配模型与额度；让管理员能够看到用量、异常和上游账号状态。
...
| B 团队试点 | 统一入口 + New API/CPA 现有页面 | 5–10 人、独立 Key、软额度、用量和异常记录 | 连续运行两周，数据可追溯 |
| C 运营版本 | 自建统一运营控制台 + 两个后台 | 组织、用途、预算、告警、审计和配置变更集中管理 | 管理员日常工作不需要反复切换后台 |
...
统一控制台初期不增加业务数据库。它读取 New API 的人员、Key、用量和日志，读取 CPA 的账号状态和健康信息；只有当需要跨系统审计、告警去重或历史快照时，才增加 MySQL/PostgreSQL 中的独立运营表。
...
姓名、邮箱、部门、岗位、在职状态、默认用途、允许模型、月度成本点数目标、告警状态和最后使用时间。

### Key

Key 名称、所属人员、用途、允许模型、渠道分组、创建时间、过期时间、启用状态、最近使用时间。页面只显示脱敏 Key，完整 Key 只在创建成功时显示一次。
...
| 角色 | 可见范围 | 可执行操作 |
| --- | --- | --- |
| Root | 全部人员、渠道和日志 | 系统配置、凭据配置、备份和权限管理 |
| 管理员 | 试点人员、Key、用量和告警 | 创建/停用 Key、调整软目标、查看日志 |
...
左侧显示姓名、部门、岗位和在职状态；右侧显示本月目标、已用点数、剩余比例、请求数、Token、成功率和最近调用时间。下方提供 Key 列表和允许模型列表。

管理员可以停用人员、停用 Key、调整软目标和查看脱敏调用记录，但不能在员工页面直接查看完整凭据。
...
创建 Key 时要求填写人员、用途、允许模型、过期时间和备注。删除或轮换前必须二次确认，并写入操作审计。
...
6. 连续运行两周，记录失败率、延迟、上游刷新、记录连续性和备份还原结果。
7. 试点通过后再开发统一运营控制台；前端先只读聚合，确认接口稳定后再开放 Key 和预算写操作。
8. 从当前电脑迁移到长期在线的公司内网服务器，再配置 HTTPS、访问控制和定期备份。
