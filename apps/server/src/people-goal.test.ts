...
describe('people model whitelist', () => {
  it('updates local policy, active keys, and audit with idempotent replay', async () => {
    const app = buildApp({ databasePath: ':memory:', probeNewApi: reachableNewApi })
    apps.push(app)
    const login = await app.inject({ method: 'POST', url: '/api/auth/login', payload: { username: 'admin', password: 'admin-demo' } })
    const cookie = cookieHeader(login.headers['set-cookie'])
    const csrfToken = cookieValue(login.headers['set-cookie'], 'ai_ops_csrf')
    const body = { idempotencyKey: 'person-models-1a2b3c4d', models: ['ecommerce-copy', 'ecommerce-pro-lab'], reason: '本地演示需要开放实验模型', acknowledgeImpact: true }

    const updated = await app.inject({ method: 'PATCH', url: '/api/people/person-lin/models', headers: { cookie, 'x-csrf-token': csrfToken }, payload: body })
    expect(updated.statusCode).toBe(200)
    expect(updated.json()).toMatchObject({ person: { id: 'person-lin', name: '林筱雨' }, keysUpdated: 2, operation: { idempotent: false, auditEventId: 'audit-person-models-1a2b3c4d' } })
    expect(updated.json().models).toEqual(expect.arrayContaining([
      expect.objectContaining({ alias: 'ecommerce-copy', allowed: true }),
      expect.objectContaining({ alias: 'ecommerce-pro-lab', allowed: true }),
      expect.objectContaining({ alias: 'ecommerce-general', allowed: false }),
    ]))

    const detail = await app.inject({ method: 'GET', url: '/api/people/person-lin', headers: { cookie } })
    expect(detail.statusCode).toBe(200)
    expect(detail.json().models.filter((model: { allowed: boolean }) => model.allowed).map((model: { alias: string }) => model.alias)).toEqual(body.models)

    const keys = await app.inject({ method: 'GET', url: '/api/keys?owner=person-lin&page=1&pageSize=20', headers: { cookie } })
    expect(keys.statusCode).toBe(200)
    expect(keys.json().items.filter((item: { status: string }) => item.status === 'active').every((item: { models: string[] }) => JSON.stringify(item.models) === JSON.stringify(body.models))).toBe(true)

    const replay = await app.inject({ method: 'PATCH', url: '/api/people/person-lin/models', headers: { cookie, 'x-csrf-token': csrfToken }, payload: body })
    expect(replay.statusCode).toBe(200)
    expect(replay.json()).toMatchObject({ keysUpdated: 2, operation: { idempotent: true, auditEventId: 'audit-person-models-1a2b3c4d' } })

    const audit = await app.inject({ method: 'GET', url: '/api/audit-events/audit-person-models-1a2b3c4d', headers: { cookie } })
    expect(audit.statusCode).toBe(200)
    expect(audit.json()).toMatchObject({ event: { id: 'audit-person-models-1a2b3c4d', resource: { id: 'person-lin' } } })
    expect(JSON.stringify(audit.json())).not.toContain(body.reason)

    const invalid = await app.inject({ method: 'PATCH', url: '/api/people/person-lin/models', headers: { cookie, 'x-csrf-token': csrfToken }, payload: { ...body, idempotencyKey: 'person-models-5e6f7g8h', models: ['ecommerce-not-real'] } })
    expect(invalid.statusCode).toBe(400)
    expect(invalid.json().error.code).toBe('MODEL_NOT_AVAILABLE')
  })
})
