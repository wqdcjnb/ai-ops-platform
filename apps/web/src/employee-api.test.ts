...
describe('employee API contracts', () => {
  it('accepts only self-scoped profiles and masked Keys', () => {
    const profile = { meta, scope, person: { id: 'person-lin', name: '林梓雨', initials: '林', employeeCode: 'OPS-017', department: '内容运营', role: 'employee', status: 'active', manager: '负责人', joinedAt: '2025-03-17' }, support: { contact: '联系管理员', serviceHours: '工作日', temporaryQuotaRequestAvailable: false, notice: '二期' }, commonErrors: [{ code: 'AUTH_INVALID', title: '鉴权失败', explanation: 'Key 无效', action: '检查配置', internalDetailAvailable: false }] }
    ...
    const keys = { meta, scope, items: [{ id: 'key-1', masked: 'sk-ops••••••1234', purpose: '文案', alias: 'copy', allowedModels: ['文案模型'], status: 'active', createdAt: '2026-01-01T00:00:00.000Z', expiresAt: '2027-01-01T00:00:00.000Z', lastUsedAt: null, secretAvailable: false, rotateAvailable: false }], connection: { baseUrl: 'http://127.0.0.1:3000/v1', credentialDelivery: '受控交付', upstreamDetailsAvailable: false, guides: [{ id: 'codex', name: 'Codex', description: '连接', steps: ['配置'] }] } }
  ...
  it('rejects hard blocking and exposed model internals', () => {
    const usage = { meta: { ...meta, period: '7d' }, scope, summary: { pointsUsed: 100, pointsTarget: 1000, pointsRemaining: 900, usagePercent: 10, requests: 5, tokens: 1000, successRate: 100, softTarget: true, requestBlockingEnabled: false }, trend: [{ date: '2026-09-15', points: 100, requests: 5 }], purposes: [{ id: 'copy', name: '文案', points: 100, percent: 100 }] }
    ...
    const models = { meta, scope, items: [{ id: 'model-1', alias: 'copy', name: '文案模型', purpose: '文案', description: '说明', capabilityTags: ['中文'], contextLabel: '长文本', status: 'available', useAdvice: '不含隐私', providerAvailable: false, actualModelAvailable: false, channelAvailable: false }] }
