...
const payload = { meta: { source: 'partial', generatedAt: '2026-09-15T10:00:00.000Z', notice: '部分实时' }, access: { currentRole: 'super_admin', serverRbacVerified: false, writeAllowed: false, notice: '只读' }, summary: { sections: 6, roles: 1, servicesOnline: 4, servicesTotal: 4, enabledFeatures: 0, backupsVerified: 0 }, organization: { source: 'demo', company: '测试公司', departments: 1, people: 1, roles: [{ id: 'super_admin', name: '超级管理员', memberCount: 1, dataScope: '全公司', permissionSummary: '全部', highPrivilege: true }] }, businessRules: { source: 'demo', version: 'draft', verified: false, items: [{ id: 'timezone', label: '时区', value: 'Asia/Shanghai', impact: '账期', status: 'fixed' }] }, connections: { source: 'live', items: [connection('bff'), connection('new-api'), connection('cpa'), connection('docs')] }, retention: { source: 'demo', cleanupJobVerified: false, items: [{ id: 'metadata', label: '元数据', days: 180, appliesTo: '调用日志', cleanupState: 'unverified', minimumNecessary: true }] }, features: { source: 'configuration', items: [{ id: 'writes', label: '写操作', enabled: false, editable: false, reason: '未验收', risk: 'high' }] }, backup: { source: 'unverified', configured: false, storageTargetConfigured: false, lastBackupAt: null, lastVerifiedAt: null, lastRestoreDrillAt: null, browserDownloadAllowed: false, notice: '未配置' } }

describe('settings API contract', () => {
  it('accepts mixed live and explicitly unverified settings data', () => {
