...
describe('alert API contracts', () => {
  it('validates explicit filters, summaries and safe event metadata', () => {
    expect(alertFiltersSchema.safeParse({ search: '', severity: 'critical', status: 'open', source: 'error_rate', environment: 'production', page: 1, pageSize: 10 }).success).toBe(true)
    expect(alertFiltersSchema.safeParse({ search: '', severity: 'fatal', status: 'open', source: 'error_rate', environment: 'production', page: 0, pageSize: 10 }).success).toBe(false)
