...
export const auditFiltersSchema = z.object({ period: periodSchema, search: z.string().max(80), eventId: z.union([z.literal(''), z.string().regex(/^audit-[a-z0-9-]{1,80}$/)]).default(''), actor: z.string(), action: z.union([z.literal('all'), actionSchema]), resource: z.union([z.literal('all'), resourceTypeSchema]), result: z.union([z.literal('all'), resultStatusSchema]), source: z.union([z.literal('all'), sourceTypeSchema]), sourceSystem: z.union([z.literal('all'), sourceSystemSchema]).default('all'), page: z.number().int().positive(), pageSize: z.number().int().min(5).max(50) })
...
const metaSchema = z.object({ source: z.enum(['demo', 'database']), generatedAt: z.string().datetime(), period: periodSchema, notice: z.string(), sources: z.array(z.object({ id: sourceSystemSchema, label: z.string(), state: sourceStateSchema, count: z.number().int().nonnegative(), notice: z.string() })).optional() })
