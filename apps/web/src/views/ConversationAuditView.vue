          ...
          <section class="conversation-detail-column">
            ...
            <template v-else>
              ...
              <div v-else class="conversation-content"><div class="conversation-demo-warning"><IconAlertTriangle :size="17" /><div><strong>合成演示内容 · 未解密真实正文</strong><p>{{ access.meta.notice }}</p></div></div><header class="conversation-content-header"><div><h3>{{ access.content.conversationTitle }}</h3><p>{{ access.record.metrics.turns }} 轮 · {{ access.record.metrics.toolCalls }} 次工具 · {{ access.record.metrics.totalTokens.toLocaleString('zh-CN') }} Token</p></div><span :class="{ warning: !access.content.redactionPassed }">{{ access.content.redactionPassed ? '脱敏通过' : '需要复核' }}</span></header>
                ...
                <section class="conversation-retention"><IconClock :size="17" /><div><strong>计划于 {{ dateTime(access.retention.expiresAt) }} 到期</strong><p>{{ access.retention.notice }}</p></div></section><section class="conversation-linked-usage" :class="{ unavailable: !access.linkedUsage.linkVerified }"><IconDatabase :size="17" /><div><strong>{{ access.linkedUsage.linkVerified ? '已关联 SQLite 模拟用量元数据' : '尚无 SQLite 模拟用量关联' }}</strong><p>{{ access.linkedUsage.notice }}</p></div><div class="conversation-linked-usage-actions"><code>{{ access.linkedUsage.usageRequestId ?? '无模拟映射' }}</code><a v-if="access.linkedUsage.linkVerified && access.linkedUsage.usageRequestId" class="text-button" :href="`/usage?requestId=${encodeURIComponent(access.linkedUsage.usageRequestId)}&origin=conversation_audit`">查看模拟用量</a></div></section><footer class="conversation-content-actions"><span>访问记录 {{ access.access.accessRecordId }} · {{ access.access.persisted ? 'SQLite 元数据已记录，原因原文不保存' : '未持久化' }}</span><a v-if="access.access.auditEventId" class="btn btn-white" :href="`/audit?eventId=${encodeURIComponent(access.access.auditEventId)}&origin=conversation_access`" :aria-label="`查看 ${access.access.auditEventId} 操作审计`"><IconShieldCheck :size="15" />查看操作审计</a><button class="btn btn-white" disabled>复制</button><button class="btn btn-white" disabled>导出</button><button class="btn danger" disabled><IconBan :size="15" />删除</button></footer>
              </div>
