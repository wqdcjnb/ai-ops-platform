...
<template>
  <div class="dashboard conversation-audit-dashboard">
    ...
    <template v-else-if="data">
      <section class="conversation-scope-banner"><span><IconShieldCheck :size="19" /></span><div><strong>默认不采集 · {{ data.scope.activePolicies }} 条演示策略生效</strong><p>{{ data.scope.notice }}</p></div><div class="conversation-scope-facts"><em>最近到期 {{ expiryText(data.scope.nearestExpiryAt) }}</em><em>服务端 RBAC 已验证</em><em>{{ data.scope.accessAuditPersisted ? '访问元数据已落库' : '访问审计未持久化' }}</em></div></section>
