...
import {
  IconAlertTriangle, IconChevronRight, IconCircleCheck, IconClock, IconFilter,
  IconKey, IconRefresh, IconSearch, IconServer2, IconShieldCheck, IconX,
...
const summaryCards = computed(() => {
  ...
  return [
    { label: '正式账号', value: value?.total ?? '—', hint: '正式环境配置', icon: IconServer2, tone: 'teal' },
...
<template>
  <div class="dashboard upstreams-dashboard">
    <section class="page-heading"><div><div class="eyebrow">UPSTREAM ACCOUNTS</div><h1>上游账号</h1><p>统一查看正式环境账号的认证、窗口与异常状态。</p></div><div class="heading-actions"><span class="updated-at">更新于 {{ updatedAt }}</span><button class="btn btn-white refresh-button" :disabled="isLoading" @click="loadData"><IconRefresh :size="17" :class="{ spinning: isLoading }" />刷新</button><button class="btn" disabled title="凭据仅通过受保护部署流程配置"><IconKey :size="16" />配置凭据</button></div></section>

    <div v-if="upstreams" class="source-banner"><span>正式配置快照</span>{{ upstreams.meta.notice }}</div>
    ...
    <template v-else-if="upstreams">
      <section class="panel upstream-list-panel"><header class="panel-header"><div><span class="panel-title">正式账号清单</span><span class="panel-subtitle">凭据仅显示配置和验证结果，不显示掩码或片段</span></div><span class="source-tag live">只读快照</span></header>
      <form class="upstream-filters" @submit.prevent="applyFilters"><label class="upstream-search"><IconSearch :size="16" /><input v-model="search" aria-label="搜索上游账号" maxlength="60" type="search" placeholder="搜索账号、供应商或模型" /></label><label><IconServer2 :size="15" /><select v-model="type" @change="applyFilters"><option value="all">全部类型</option><option value="official_api">官方 API</option></select></label><label><IconFilter :size="15" /><select v-model="status" @change="applyFilters"><option value="all">全部状态</option><option value="healthy">健康</option><option value="degraded">需关注</option><option value="auth_required">认证异常</option><option value="offline">离线</option><option value="unconfigured">未配置</option></select></label><span class="realtime-search-hint" aria-live="polite">输入即搜索</span><button class="text-button" type="button" @click="clearFilters">清除</button></form>

        <div v-if="visibleItems.length" class="upstream-account-grid">
          <button v-for="item in visibleItems" :key="item.id" class="upstream-account-card" :aria-label="`查看 ${item.name} 账号详情`" @click="openDetail(item)">
            <div class="upstream-card-head"><span><IconServer2 :size="19" /></span><div><strong>{{ item.name }}</strong><small>{{ item.provider }} · {{ item.models.join(' / ') }}</small></div><span class="route-status" :class="`status-${item.status}`"><i />{{ statusText[item.status] }}</span></div>
        ...
        <div v-else class="people-empty"><IconServer2 :size="24" /><strong>没有符合条件的正式账号</strong><span>调整账号类型、状态或搜索内容。</span><button class="text-button" @click="clearFilters">清除筛选</button></div>
      </section>
      <footer class="page-footer">数据来源：正式配置快照 · 浏览器永不接触 API Key、OAuth Token 或管理密钥</footer>
    </template>
    ...
    <div v-if="selected" class="drawer-backdrop" @click.self="selected = null"><aside class="model-drawer upstream-drawer" role="dialog" aria-modal="true" aria-label="上游账号详情"><header><div><span class="source-tag live">正式配置</span><h2>上游账号详情</h2></div><button class="icon-button" aria-label="关闭详情" @click="selected = null"><IconX :size="20" /></button></header>
      <section class="model-drawer-hero upstream"><span><IconServer2 :size="22" /></span><div><strong>{{ selected.name }}</strong><code>{{ selected.provider }}</code><small>{{ selected.type === 'official_api' ? '官方 API 正式账号' : 'OAuth 正式账号' }}</small></div><span class="route-status" :class="`status-${selected.status}`"><i />{{ statusText[selected.status] }}</span></section>
      ...
      <section v-else class="drawer-section"><h3>OAuth 认证与窗口</h3><dl class="model-facts"><div><dt>认证有效期</dt><dd>{{ dateText(selected.auth?.expiresAt) }}</dd></div><div><dt>最后刷新</dt><dd>{{ dateText(selected.auth?.lastRefreshedAt) }}</dd></div></dl><div class="account-window-list"><article v-for="window in selected.windows" :key="window.id"><div><strong>{{ window.label }}</strong><em>{{ window.usedPercent }}% 已用</em></div><span><i :class="percentTone(window.usedPercent)" :style="{ width: `${window.usedPercent}%` }" /></span><small>{{ timeText(window.resetsAt) }} 重置</small></article></div><div class="cooldown-state" :class="{ active: selected.cooldown?.active }"><IconClock :size="17" /><span><strong>{{ selected.cooldown?.active ? '账号处于冷却' : '当前无冷却' }}</strong><small v-if="selected.cooldown?.active">{{ selected.cooldown.reason }} · {{ timeText(selected.cooldown.until!) }} 结束</small><small v-else>当前可以承载请求</small></span></div></section>
