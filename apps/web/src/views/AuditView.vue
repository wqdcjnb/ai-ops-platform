...
const resourceText = { session: '会话', person: '人员', key: 'Key', quota: '额度', route: '路由', export: '导出', settings: '设置', alert: '告警' }
const roleText = { super_admin: '超级管理员', admin: '管理员', system: '系统任务' }
...
<template>
  <div class="dashboard audit-dashboard">
    ...
    <div v-if="audit" class="source-banner"><span>DEMO</span>{{ audit.meta.notice }}</div>
    ...
    <template v-else-if="audit">
      <section class="audit-integrity-banner"><span><IconShieldLock :size="19" /></span><div><strong>审计证据只读</strong><p>{{ audit.retention.notice }}</p></div><div><em><IconDatabase :size="14" />追加写入待验证</em><em><IconLock :size="14" />页面不可删除</em></div></section>

      <section class="panel audit-main-panel"><header class="panel-header"><div><span class="panel-title">操作事件</span><span class="panel-subtitle">登录、人员、Key、额度、路由、导出、告警与管理配置</span></div><span class="source-tag demo">DEMO</span></header>
        <form class="audit-filters" @submit.prevent="applyFilters"><label class="audit-search"><IconSearch :size="15" /><input v-model="search" maxlength="80" type="search" placeholder="请求 ID、事件 ID、操作人、对象或摘要" /></label><label><select v-model="period" @change="applyFilters"><option value="today">今天</option><option value="7d">近 7 天</option><option value="30d">近 30 天</option></select></label><label><IconUser :size="14" /><select v-model="actor" @change="applyFilters"><option value="all">全部操作人</option><option v-for="option in audit.options.actors" :key="option.id" :value="option.id">{{ option.label }}</option></select></label><label><select v-model="action" @change="applyFilters"><option value="all">全部动作</option><option value="login">登录</option><option value="create">创建</option><option value="update">更新</option><option value="disable">停用</option><option value="rotate">轮换</option><option value="export">导出</option><option value="acknowledge">确认</option><option value="view">查看</option></select></label><label><select v-model="resource" @change="applyFilters"><option value="all">全部对象</option><option value="session">会话</option><option value="person">人员</option><option value="key">Key</option><option value="quota">额度</option><option value="route">路由</option><option value="export">导出</option><option value="settings">设置</option><option value="alert">告警</option></select></label><label><select v-model="result" @change="applyFilters"><option value="all">全部结果</option><option value="success">成功</option><option value="failed">失败</option><option value="denied">已拒绝</option></select></label><label><select v-model="source" @change="applyFilters"><option value="all">全部来源</option><option value="web">管理页面</option><option value="api">管理接口</option><option value="system">系统任务</option></select></label><button class="btn filter-submit" type="submit">查询</button><button class="text-button" type="button" @click="clearFilters">清除</button></form>
      ...
      <footer class="page-footer">数据来源：DEMO · 只展示字段级摘要 · 不保存完整 Key、认证信息、请求正文或对话正文 · 页面无删除入口</footer>
    </template>

    <div v-if="detail" class="drawer-backdrop" @click.self="detail = null"><aside class="model-drawer audit-drawer" role="dialog" aria-modal="true" aria-label="审计事件详情"><header><div><span class="source-tag demo">DEMO</span><h2>审计事件详情</h2></div><button class="icon-button" aria-label="关闭详情" @click="detail = null"><IconX :size="20" /></button></header>
      ...
      <section class="drawer-section"><h3>请求关联</h3><div class="audit-request-card"><code>{{ detail.request.requestId }}</code><span>HTTP {{ detail.request.responseCode }} · {{ detail.request.durationMs }}ms</span><small>链路状态：演示数据，尚未验证真实 x-request-id 透传</small></div><div v-if="detail.relatedAuditIds.length" class="related-requests"><code v-for="id in detail.relatedAuditIds" :key="id">{{ id }}</code></div></section>
      <section class="audit-integrity-detail"><IconShieldLock :size="19" /><div><strong>不可篡改状态尚未验证</strong><p>{{ detail.integrity.notice }}</p><small>删除：禁止 · 追加写入：待验证 · 哈希链：待验证</small></div></section>
