...
function linkedRouteFilterFromLocation() { return ['route', 'channel', 'upstream', 'key', 'person'].includes(initialQuery?.get('resource') ?? '') && Boolean(initialQuery?.get('search')?.trim()) }
...
const linkedAuditResourceLabel = computed(() => resource.value === 'channel' ? '模型渠道' : resource.value === 'upstream' ? '上游账号' : resource.value === 'key' ? 'Key' : resource.value === 'person' ? '人员' : '路由')
