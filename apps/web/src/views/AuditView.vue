...
function linkedRouteFilterFromLocation() { return ['route', 'channel', 'upstream'].includes(initialQuery?.get('resource') ?? '') && Boolean(initialQuery?.get('search')?.trim()) }
...
const linkedAuditResourceLabel = computed(() => resource.value === 'channel' ? '模型渠道' : resource.value === 'upstream' ? '上游账号' : '路由')
