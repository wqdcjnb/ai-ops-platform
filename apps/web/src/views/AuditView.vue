...
function initialResourceFromLocation(): AuditFilters['resource'] {
  ...
  return ['all', 'session', 'authorization', 'gateway_request', 'person', 'key', 'quota', 'route', 'channel', 'upstream', 'export', 'settings', 'alert', 'conversation'].includes(value)
...
const resourceText = { session: '会话', authorization: '权限校验', gateway_request: '网关请求', person: '人员', key: 'Key', quota: '额度', route: '路由', channel: '模型渠道', upstream: '上游账号', export: '导出', settings: '设置', alert: '告警', conversation: '对话审计' }
...
const detailSourceLabel = computed(() => detail.value?.meta.source === 'database' ? 'SQLite · 操作与网关事件' : '演示数据')
