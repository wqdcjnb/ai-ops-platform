...
<template>
  <div class="dashboard audit-dashboard">
    ...
    <div v-if="audit" class="source-banner"><span>{{ audit.meta.source.toUpperCase() }}</span>{{ audit.meta.notice }}</div>
    ...
    <template v-else-if="audit">
      ...
      <section class="panel audit-main-panel"><header class="panel-header"><div><span class="panel-title">操作事件</span><span class="panel-subtitle">登录、人员、Key、额度、路由、导出、告警与管理配置</span></div><span class="source-tag" :class="audit.meta.source">{{ audit.meta.source.toUpperCase() }}</span></header>
      ...
      <footer class="page-footer">数据来源：{{ audit.meta.source.toUpperCase() }} · 只展示字段级摘要 · 不保存完整 Key、认证信息、请求正文或对话正文 · 页面无删除入口</footer>
    </template>

    <div v-if="detail" class="drawer-backdrop" @click.self="detail = null"><aside class="model-drawer audit-drawer" role="dialog" aria-modal="true" aria-label="审计事件详情"><header><div><span class="source-tag" :class="detail.meta.source">{{ detail.meta.source.toUpperCase() }}</span><h2>审计事件详情</h2></div><button class="icon-button" aria-label="关闭详情" @click="detail = null"><IconX :size="20" /></button></header>
