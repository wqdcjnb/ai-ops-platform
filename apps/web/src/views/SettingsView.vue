...
const roleText = { super_admin: '超级管理员', admin: '运营管理员', department_lead: '部门负责人', finance: '财务只读', employee: '员工' }
    ...
    <template v-else-if="settings">
      ...
      <section class="settings-workbench">
        ...
        <div class="settings-section-content">
          <section v-if="activeSection === 'organization'" class="panel settings-section-panel"><header class="settings-panel-heading"><div><span class="source-tag demo">DEMO</span><h2>组织与角色</h2><p>{{ settings.organization.company }} · {{ settings.organization.departments }} 个部门 · {{ settings.organization.people }} 人</p></div><button class="btn btn-white" disabled>管理角色</button></header><div class="settings-role-table"><div class="settings-table-head"><span>角色</span><span>成员</span><span>数据范围</span><span>权限摘要</span><span>级别</span></div><article v-for="role in settings.organization.roles" :key="role.id"><span class="settings-role-name"><i>{{ role.name.slice(0, 1) }}</i><strong>{{ roleText[role.id] }}</strong></span><span>{{ role.memberCount }} 人</span><span>{{ role.dataScope }}</span><span>{{ role.permissionSummary }}</span><span class="privilege-tag" :class="{ high: role.highPrivilege }">{{ role.highPrivilege ? '高权限' : '普通' }}</span></article></div><footer class="settings-section-note"><IconAlertTriangle :size="15" />角色人数和范围为演示数据；真实高权限变更必须二次确认并写入操作审计。</footer></section>

          <section v-else-if="activeSection === 'business'" class="panel settings-section-panel"><header class="settings-panel-heading"><div><span class="source-tag demo">DEMO</span><h2>业务口径</h2><p>配置版本 {{ settings.businessRules.version }} · 尚未完成财务和网关对账</p></div><button class="btn btn-white" disabled>创建新版本</button></header><div class="business-rule-grid"><article v-for="item in settings.businessRules.items" :key="item.id"><header><strong>{{ item.label }}</strong><span :class="item.status">{{ item.status === 'fixed' ? '已固定' : '待验证' }}</span></header><p>{{ item.value }}</p><small>影响：{{ item.impact }}</small></article></div><footer class="settings-section-note"><IconAlertTriangle :size="15" />口径变更必须版本化，提交前展示影响范围，并在生效和回滚时分别记录审计。</footer></section>
          ...
          <section v-else-if="activeSection === 'retention'" class="panel settings-section-panel"><header class="settings-panel-heading"><div><span class="source-tag demo">DEMO</span><h2>数据留存</h2><p>采用最短必要期限；后台清理任务尚未接入和验证。</p></div><button class="btn btn-white" disabled>调整留存</button></header><div class="retention-list"><article v-for="item in settings.retention.items" :key="item.id"><span class="retention-days"><strong>{{ item.days }}</strong><small>天</small></span><div><strong>{{ item.label }}</strong><p>{{ item.appliesTo }}</p></div><span class="cleanup-state">{{ item.cleanupState === 'not_configured' ? '清理未配置' : '清理待验证' }}</span></article></div><footer class="settings-section-note"><IconAlertTriangle :size="15" />修改留存前需展示删除影响；对话正文到期后仅保留不含内容的删除证明。</footer></section>

          <section v-else-if="activeSection === 'features'" class="panel settings-section-panel"><header class="settings-panel-heading"><div><span class="source-tag unverified">CONFIG</span><h2>功能开关</h2><p>服务端安全条件未验收的能力统一保持关闭。</p></div><button class="btn btn-white" disabled>管理开关</button></header><div class="feature-flag-list"><article v-for="feature in settings.features.items" :key="feature.id"><span class="feature-icon"><IconSwitch :size="17" /></span><div><strong>{{ feature.label }}</strong><p>{{ feature.reason }}</p></div><span class="risk-tag" :class="feature.risk">{{ feature.risk === 'high' ? '高风险' : feature.risk === 'medium' ? '中风险' : '低风险' }}</span><span class="readonly-switch" :class="{ on: feature.enabled }" role="switch" :aria-checked="feature.enabled" aria-disabled="true"><i /></span><em>{{ feature.enabled ? '已开启' : '已关闭' }}</em></article></div><footer class="settings-section-note success"><IconCheck :size="15" />当前 5 项未验收能力均由服务端配置保持关闭，前端不提供绕过入口。</footer></section>

          <section v-else class="panel settings-section-panel"><header class="settings-panel-heading"><div><span class="source-tag unverified">UNVERIFIED</span><h2>备份状态</h2><p>只展示作业元数据，不允许从浏览器下载数据库。</p></div><button class="btn btn-white" disabled><IconRestore :size="15" />发起恢复演练</button></header><div class="backup-empty"><span><IconArchive :size="27" /></span><h3>尚未配置平台数据库备份</h3><p>{{ settings.backup.notice }}</p><div><article><small>存储目标</small><strong>未配置</strong></article><article><small>最近备份</small><strong>无记录</strong></article><article><small>最近校验</small><strong>无记录</strong></article><article><small>恢复演练</small><strong>无记录</strong></article></div><button class="btn" disabled>配置受保护备份作业</button></div><footer class="settings-section-note"><IconDatabase :size="15" />备份文件、数据库、密钥和认证资料不得通过浏览器直接下载。</footer></section>
        </div>
      </section>
      <footer class="page-footer">数据来源：服务状态 LIVE，其余 DEMO / UNVERIFIED · 设置写入、凭据查看、备份下载与恢复操作均禁用</footer>
    </template>
  </div>
</template>
