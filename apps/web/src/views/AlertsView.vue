...
import {
  ...
  IconChevronRight, IconCircleCheck, IconClock, IconFilter, IconRefresh, IconSearch, IconShieldCheck, IconX,
} from '@tabler/icons-vue'
import { acknowledgeLocalAlert, AlertsApiError, closeLocalAlert, fetchAlertDetail, fetchAlerts, fetchAlertSummary, type AlertAcknowledgeBody, type AlertCloseBody, type AlertDetail, type AlertEvent, type AlertFilters, type AlertsResponse, type AlertSummary } from '../alerts-api'
...
const environment = ref<AlertFilters['environment']>('production')
const page = ref(1)
const pageSize = 10
...
const sourceLabel = computed(() => summary.value?.meta.simulated ? 'SQLite · 只读快照' : 'SQLite · 网关数据')
const environmentText: Record<AlertEvent['environment'], string> = { production: '生产', experiment: '生产' }
...
function clearFilters() { search.value = ''; subjectId.value = ''; alertId.value = ''; severity.value = 'all'; status.value = 'all'; source.value = 'all'; environment.value = 'production'; removeRelatedAlertQuery(); applyFilters() }
...
async function loadData() {
  ...
  errorMessage.value = ''
  try {
    const [summaryValue, alertsValue] = await Promise.all([fetchAlertSummary(next.signal), fetchAlerts(filters(), next.signal)])
...
<template>
  <div class="dashboard alerts-dashboard">
    ...
    <div v-if="summary" class="source-banner"><span>{{ sourceLabel }}</span>{{ summary.meta.notice }}</div>
    ...
    <div v-if="!summary && !errorMessage" class="panel data-state"><div class="state-icon"><IconRefresh :size="22" class="spinning" /></div><div><strong>正在聚合告警</strong><p>正在读取事件和通知配置状态…</p></div></div>
    ...
    <template v-else-if="summary && alerts">
      ...
      <section class="panel alert-workbench">
          ...
          <div v-else class="people-empty"><IconBellOff :size="25" /><strong>没有符合条件的告警事件</strong><span>调整严重度、状态或来源筛选。</span><button class="text-button" @click="clearFilters">清除筛选</button></div>
      ...
      <footer class="page-footer">数据来源：SQLite 只读快照 · 可确认或关闭本地告警；规则编辑、新建规则、静默、外部通知和真实事件处置保持禁用 · 不返回凭证或上游完整错误正文</footer>
    </template>
...
.alert-tabs > strong { height: 48px; display: inline-flex; align-items: center; border-bottom: 2px solid var(--brand, #19808a); color: var(--brand, #19808a); font-size: 9px; }
.alert-tabs > span { margin: 0 0 17px 2px; padding: 2px 5px; border-radius: 9px; color: #6f7e88; background: #edf1f3; font-size: 7px; }
