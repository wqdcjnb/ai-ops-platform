...
<template>
  <div class="dashboard usage-dashboard">
    ...
    <div v-if="usage" class="source-banner"><span>DEMO</span>{{ usage.meta.notice }}</div>
    ...
    <template v-else-if="usage">
      ...
      <section class="panel usage-main-panel"><header class="panel-header"><div><span class="panel-title">调用记录</span><span class="panel-subtitle">只读脱敏元数据 · 默认无请求与响应正文</span></div><span class="source-tag demo">DEMO</span></header>
        <form class="usage-filters" @submit.prevent="applyFilters">
          <label class="usage-search"><span>请求或 Key</span><div><IconSearch :size="15" /><input v-model="search" maxlength="80" type="search" placeholder="请求 ID、掩码 Key、人员或别名" /></div></label>
          <label><span>日期</span><select v-model="period" @change="applyFilters"><option value="today">今天</option><option value="7d">近 7 天</option><option value="30d">近 30 天</option></select></label>
          <label><span>人员</span><select v-model="person" @change="applyFilters"><option value="all">全部人员</option><option v-for="item in usage.options.people" :key="item.id" :value="item.id">{{ item.label }}</option></select></label>
          <label><span>部门</span><select v-model="department" @change="applyFilters"><option value="all">全部部门</option><option v-for="item in usage.options.departments" :key="item.id" :value="item.id">{{ item.label }}</option></select></label>
          <label><span>用途</span><select v-model="purpose" @change="applyFilters"><option value="all">全部用途</option><option v-for="item in usage.options.purposes" :key="item.id" :value="item.id">{{ item.label }}</option></select></label>
          <label><span>Key</span><select v-model="key" @change="applyFilters"><option value="all">全部 Key</option><option v-for="item in usage.options.keys" :key="item.id" :value="item.id">{{ item.label }}</option></select></label>
          <label><span>模型</span><select v-model="model" @change="applyFilters"><option value="all">全部模型</option><option v-for="item in usage.options.models" :key="item.id" :value="item.id">{{ item.label }}</option></select></label>
          <label><span>渠道</span><select v-model="channel" @change="applyFilters"><option value="all">全部渠道</option><option v-for="item in usage.options.channels" :key="item.id" :value="item.id">{{ item.label }}</option></select></label>
          <label><span>结果</span><select v-model="status" @change="applyFilters"><option value="all">全部结果</option><option value="succeeded">成功</option><option value="failed">失败</option><option value="cancelled">已取消</option></select></label>
          <label><span>成本口径</span><select v-model="costType" @change="applyFilters"><option value="all">全部口径</option><option value="official_actual">官方实际</option><option value="platform_estimate">平台估算</option><option value="cpa_estimate">CPA 估算</option></select></label>
      ...
      <footer class="page-footer">数据来源：DEMO · 日志只含脱敏元数据 · 不保存认证 Header、完整 Key 或未经授权的对话正文</footer>
    </template>

    <div v-if="detail" class="drawer-backdrop" @click.self="detail = null"><aside class="model-drawer usage-drawer" role="dialog" aria-modal="true" aria-label="调用详情"><header><div><span class="source-tag demo">DEMO</span><h2>调用详情</h2></div><button class="icon-button" aria-label="关闭详情" @click="detail = null"><IconX :size="20" /></button></header>
      ...
      <footer class="drawer-actions"><button class="btn btn-white" disabled><IconFileAnalytics :size="16" />查看对话审计</button><button class="btn" disabled><IconFileAnalytics :size="16" />导出记录</button></footer>
    </aside></div>
  </div>
</template>
