...
import {
  ...
  IconCircleCheck, IconClock, IconFileAnalytics, IconRefresh, IconSearch, IconShieldCheck, IconX,
...
const sourceLabel = computed(() => usage.value?.meta.source !== 'database' ? '正在读取' : usage.value.meta.simulated ? 'SQLite · 脱敏快照' : 'SQLite · 网关数据')
const linkedUsageTitle = computed(() => linkedUsageOrigin.value === 'alert' ? '来自告警事件的关联调用' : '来自对话审计的关联用量')
const linkedUsageNotice = computed(() => linkedUsageOrigin.value === 'alert' ? '当前仅筛选告警明确关联的调用记录，不代表真实上游请求链路。' : '当前仅筛选系统已关联的记录，不代表完整网关请求链路。')
