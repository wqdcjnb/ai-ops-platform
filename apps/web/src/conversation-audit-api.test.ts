...
describe('conversation audit API contracts', () => {
  ...
  it('accepts only synthetic redacted access results with risky actions disabled', () => {
    const access = { meta: { source: 'demo', generatedAt: '2026-09-15T10:00:00.000Z', notice: '演示' }, record, access: { accessRecordId: 'access-demo-test-01', auditEventId: 'audit-conversation-test-01', reasonAccepted: true, persisted: true, authorizedByServerRbac: true, copyAllowed: false, exportAllowed: false, deleteAllowed: false }, content: { synthetic: true, decrypted: false, redactionPassed: true, conversationTitle: '演示会话', messages: [{ id: 'msg-1', role: 'user', label: '用户输入', occurredAt: '2026-09-15T10:00:00.000Z', text: '手机号已替换为 [PHONE_REDACTED]', redacted: true, redactionLabels: ['手机号'], tool: null }] }, retention: { expiresAt: '2026-09-16T10:00:00.000Z', cleanupState: 'scheduled', deletionProofAvailable: false, notice: '待验证' }, linkedUsage: { auditRequestId: 'req-conv-test-01', usageRequestId: 'req-demo-001', metadataEndpoint: '/api/usage/req-demo-001', linkVerified: true, source: 'synthetic_seed', notice: '合成映射' } }
